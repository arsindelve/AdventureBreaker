...
_Generated 2026-07-24T15:00:24Z · 86 finding(s)_
...
## AB-086 [CRITICAL] Floyd Bio-Lab sacrifice impossible in prod: BioLockEast.StateMachine is read-only, never serialized, resets every stateless turn -> game unwinnable  · _filed#493_

- game `planetfall` · area `Bio Lock East (Floyd sacrifice)` · category `stateless-serialization` · target_sha `128cae8`
- command: `open door (at Bio Lock East)`

BioLockEast.cs:13 StateMachine is a get-only property; DoNotSerializeReadOnlyPropertiesResolver (GameEngine.cs:1197) drops non-writable props, so the sacrifice state machine (LabSequenceState/HasWaitedOneTurnInBioLockEast/FloydHasSaidNeedToGetCard) is never written to the base64 session and resets to NotStarted every request. Floyd never says NeedToGetCard; opening the inner door hits the wrong-time death. Mini card (behind door, sacrifice-only) is unobtainable -> Planetfall unwinnable in prod. Same #472 class. Same symptom as closed #365 which mis-diagnosed it as actor-ordering; serialization cause never addressed (StateMachine still read-only on main).

...
_Generated 2026-09-12T22:34:57Z · 122 finding(s)_
...
## AB-112 [MEDIUM] Floyd companion Lambda was down (OpenAI Assistants API sunset) -- misdiagnosed by this finding as an engine/router regression; both the real outage and the misdiagnosis are now resolved  · _resolved#562_
...
CORRECTED 2026-09-07: the original diagnosis in this finding ("total regression of the companion-command router" in this repo's engine code) was wrong. Per zorkai#562's own investigation (maintainer comments 02:13Z and 13:25Z): the real cause was the Floyd companion Lambda running entirely on OpenAI's Assistants API, which OpenAI hard-sunset on 2026-08-26 -- every call returned a 404-wrapped error. This repo's ChatWithCompanion.AskCompanionAsync did not inspect the Lambda's inner statusCode and threw a NullReferenceException on the missing Results body, which Floyd.OnBeingTalkedTo's catch block (Floyd.cs:257) turned into the misleading in-character line 'Floyd tilts his head... doesn't seem to understand' -- read at the time as narrator hallucination masking an engine bug, when it was actually masking a real upstream outage. BOTH are fixed: the Lambda migrated to Chat Completions (arsindelve/Floyd#12, deployed 2026-09-07), and this repo surfaces the Lambda's own error instead of masking it as an NRE (zorkai#564, merged). Separately, this session compounded the error with a follow-up 'third confirmation' comment on #562 (posted after the fix was already deployed and the issue closed) that misread narrator-OFF 'no effect' results as evidence -- that is not a valid signal: ConversationHandler.cs:69 deliberately returns null for a present talkable NPC when generation is disabled (the #182 kill-switch), so narrator-off can never demonstrate whether Floyd's Lambda-backed conversation works. The maintainer corrected this with actual CloudWatch logs for the exact session window, showing 36 narrator-ON floyd commands all answered correctly and only the 2 narrator-OFF calls returning 'no effect'. Independently re-verified live after this correction, in the same run, in Repair Room, narrator ON: 'floyd, go north' -> canonical 'Floyd squeezes through the opening... Just a shiny fromitz board' text; 'floyd, take board' -> 'fromitz board' actually present in the structured inventory envelope afterward. The mechanic works end-to-end. Left as a medium-severity record of the real (now-fixed) Lambda outage window (~2026-08-26 to 2026-09-07), not as an open engine defect -- do not re-probe or re-file this as 'floyd commands broken' without testing with the narrator ON specifically, per the maintainer's documented test protocol on #562.

## AB-117 [MEDIUM] 'take <scenery>' bypasses scenery entirely and gets invented narrator prose, while 'get <same noun>' returns the authored refusal -- every SceneryItem.CannotBeTakenReason in every game is reachable only by phrasing luck  · _filed#577_

- game `stationfall` · area `Deck Twelve` · category `take-drop-scope` · target_sha `3aa201c`
- command: `take wall (Stationfall, Deck Twelve) -> narrator invention; get wall -> 'The bulkheads are structural, and you are not.'`

Root cause: the two phrasings land in different intent classes and only one of them ever sees scenery. When the prod parser emits a TakeIntent, GameEngine/GameEngine.cs:935 routes it to TakeOrDropInteractionProcessor.Process(TakeIntent) (GameEngine/Item/ItemProcessor/TakeOrDropInteractionProcessor.cs:66-78), which resolves against real items only; on null/NoNounMatch it hands the turn to the narrator via the TakeSomethingThatIsNotPortable request (:72-76). It never consults the location's Scenery or the game's GlobalScenery. When the parser instead emits a SimpleIntent (verb 'get'), SimpleInteractionEngine reaches LocationBase.RespondToSimpleInteraction, whose scenery branch (GameEngine/Location/LocationBase.cs:352-361) matches the noun and returns SceneryItem.CannotBeTakenReason. So the authored refusal string exists and is correct -- it is simply unreachable through the canonical verb. Confirmed engine-wide, not Stationfall-specific: Zork North of House 'get house' -> 'You can'\''t take a house with you.' but 'take house' -> falls through. This is pre-existing (the scenery take branch dates to zorkai#315), NOT a 2.10.0 regression -- but 2.10.0's whole thesis (#563) is that an improvised answer is indistinguishable from a real one and therefore worse than nothing, and it authored seven new global CannotBeTakenReason strings that a player typing the canonical 'take' never sees. The improvisation is actively wrong on the game's own facts: narrator ON, 'take wall' on Deck Twelve of the Duffy (pre-launch, aboard a ship) -> 'Ah yes, because nothing says "well-prepared Lieutenant" quite like lugging a space station wall onto your ship.' -- it invents a space station the player has not reached and promotes an Ensign Seventh Class (the game'\''s own 'examine me' text) to Lieutenant. 'take ceiling' and 'take air' improvise likewise. Note 'push wall' returning the quiet sentinel is BY DESIGN (scenery + unsupported verb -> NoVerbMatchInteractionResult -> GetGeneratedNoMatchingVerbResponse, a deliberately narrator-phrased 'you can'\''t do that to it'), so it is not part of this finding. Suggested fix: have the TakeIntent path fall back to the location/global scenery lookup before handing off to the narrator (or reroute a take whose noun matches no item but does match scenery through the SimpleIntent path), so CannotBeTakenReason answers for take/get/grab/pick up alike. TDD: assert 'take wall' and 'take house' return the authored reason with the real parser'\''s TakeIntent shape, which the deterministic test parser cannot currently catch because it emits SimpleIntent for these.

## AB-119 [MEDIUM] 2.11.0 did not fix 'enter door' in prod: still 'You cannot go that way.' (PR #573 / issue #551)  · _fixed#582_

- game `planetfall` · area `Elevator Lobby` · category `parser-routing` · target_sha `3aa201c`
- command: `enter door`

PR #573's player-facing table promises 'enter door' in the Planetfall Elevator Lobby now answers 'Do you mean...?' instead of refusing. On prod running 2.11.0 it still returns the verbatim #551 symptom 'You cannot go that way.' in 14/14 trials across two rooms and both narrator modes. The MIRRORED half of the same PR does work: 'exit door' produced the exact promised question in the same room, so the MoveEngine.TryBoardTheNamedObject routing is deployed and reachable - only the Direction.In side never fires. Decisive discriminator: in the Upper Elevator, which has exactly ONE matching door in scope and no In exit, 'enter door' still refused 3/3 - so this is not the two-door disambiguation, the enter path never reaches EnterSubLocationEngine at all (an unambiguous single door would have been rerouted through DoorReroute, or at worst answered 'You can't enter that.'). Since MoveEngine's In and Out branches share one code path, the asymmetry most likely lives in what the live parser emits for 'enter door' (direction/noun tags) versus 'exit door'. PR #573's four tests hard-code the tagged completion in LiveParserShapeTests, so a live-shape mismatch is exactly the fixture blind spot the PR itself calls out and would be invisible to the suite.

## AB-122 [MEDIUM] 2.12.0 fixed zorkai#580 but 'enter door' still only asks half the time (PR #582)  · _filed#584_

- game `planetfall` · area `Elevator Lobby` · category `parser-routing` · target_sha `0772234`
- command: `enter door`

PR #582 (fix for the issue AdventureBreaker filed as zorkai#580) is LIVE and genuinely effective: the 2.11.0 symptom 'You cannot go that way.' is now GONE, 0/14, and the decisive single-door case is a clean 5/5 - 'enter door' in the Upper Elevator walks through, where 2.11.0 refused 3/3. But the headline row is still nondeterministic: in the two-door Elevator Lobby the promised 'Do you mean...' question appears only 7/14. The other 7 split three ways: 4 SILENTLY TRAVERSE to the Upper Elevator - the exact behaviour issue #532 was filed to eliminate, and a wrong-door risk for a player who meant the lower elevator - 2 return 'You can't get there from here.' and 1 the no-handler sentinel. The 'You can't get there from here.' outcome matters for root cause: PR #582's own elimination table uses that message to RULE OUT the unresolved-direction hypothesis, so observing it in prod proves there is a second live parser shape where the direction does NOT resolve to In/Out and the command goes to destination navigation instead. #582's recovery only covers the shape where the direction resolves and the noun is dropped, so it cannot help that one. 'exit door' improved from 1/5 asking on 2.11.0 to 4/5 here. Suggest capturing the raw completions for 'enter door' across repeated prod turns - PR #581 shipped exactly the ParsedIntent/TerminalPath logging in the same release that would name each shape directly.
...
## AB-121 [LOW] 'pick up house' alone misses the scenery take path that every sibling verb reaches (PR #579 / issue #577)  · _filed#585_

- game `zork` · area `North of House` · category `parser-shape` · target_sha `0772234`
- command: `pick up house`

PR #579 claims take/get/grab/pick up/hold/acquire/snatch/carry all return the same authored scenery reason. In prod 2.12.0 the exact string 'pick up house' returns the no-handler sentinel 5/5 deterministically, while every sibling verb on the same noun returns the authored refusal. The verb is not the problem and the noun is not the problem: 'pick up boards' works 2/2 in the same room, 'pick up mailbox' works on the real item at West of House, and adding an adjective repairs it - 'pick up colonial' and 'pick up colonial house' both return the authored refusal. So only the exact two-word-verb plus bare-noun pairing misroutes. Same class as issue #580: the PR's guard tests drive TakeIntent with the noun set directly, so a live-parser shape mismatch for one phrasing is invisible to the suite. Worth a raw completion capture before any engine change.
...
## AB-114 [INFO] VERIFIED in 2.10.0 prod: zorkai#574 -- canonical 'class three activation form' phrasing now activates the spacecraft (AB-113 fixed)  · _open_

- game `stationfall` · area `Spacetruck` · category `puzzle-step` · target_sha `3aa201c`
- command: `put class three activation form in slot`

Release 2.10.0 (deploy run 34709696967, release event, completed/success 2026-09-12T18:00:03Z) verified live on the Stationfall prod backend. AB-113 / zorkai#568 regressed no more: the walkthrough's own phrasing now resolves at the autopilot slot. Driven black-box through the extracted Stationfall spine (steps 0-11, all clean), then the launch sequence continued manually with the runtime-computed course heading (course 409 from CurrentTime 4887 via ((T/50-132)^2/4)+103). Every downstream beat the bug had silenced now fires in order: engines whine, roars out of the cargo bay, fuel three-quarters, braking rockets, fuel one-quarter, space station resolves, 'Docking bay one is occupied. Defaulting to bay two.' + 'Stationfall.', score 0 -> 5. Before the fix, this command returned 'There'\''s no room.' and every subsequent wait returned only 'Time passes...'.

## AB-115 [INFO] VERIFIED in 2.10.0 prod: zorkai#571 -- 'put good in cube' (bare adjective) now performs the Course Control bedistor swap  · _open_

- game `planetfall` · area `MECH:planetary-defense-bedistor` · category `parser-gap` · target_sha `3aa201c`
- command: `put good in cube`

Release 2.10.0 verified live on the Planetfall prod backend. zorkai#550 fixed: GoodBedistor now registers the bare adjective 'good', so the bare-adjective phrasing resolves instead of collapsing to narrator prose with no state change. Verified at Course Control with the good bedistor and wide-nosed pliers in inventory and the cube open on the fused bedistor. Both phrasings confirmed from the same checkpoint (save/restore fork): the newly fixed bare 'good' and the qualified 'good bedistor' that already worked, plus the sibling guard 'take fused with pliers'. Score advanced 42 -> 48 (the 6-point Course Control repair) on both. NOTE ON SETUP: the spine replay was confounded before Course Control -- spine step 229 'open pocket' hit a live disambiguation prompt in Lab Storage ('Do you mean the patrol uniform pocket or the lab uniform pocket?', the worn patrol uniform vs the lab uniform on the rack), which consumed step 230 'take teleportation' as its answer, so the teleportation access card was never acquired and the Library/pliers/bedistor leg (steps 235-261) replayed against the wrong map. Preconditions were therefore restored with controlled white-box setup ('god mode take pliers', 'god mode take good bedistor') rather than faked or asserted; the cube survived the god-mode Repository rebuild (still open, still holding the fused bedistor, verified via state). The disambiguation collision itself is unrelated to 2.10.0 and is the known class tracked in DISAMBIGUATION_PLAN.md.

## AB-116 [INFO] VERIFIED in 2.10.0 prod: zorkai#563 -- Stationfall GlobalScenery, the new room scenery, and the spacetruck exterior object all answer live  · _open_

- game `stationfall` · area `Deck Twelve` · category `narrator-hallucination` · target_sha `3aa201c`
- command: `examine wall / examine corridor / examine spacetruck`

Release 2.10.0 verified live on the Stationfall prod backend, narrator OFF so only the engine's own authored strings can show through. All seven GlobalScenery entries answer with the exact shipped text in Deck Twelve (wall/walls, ceiling, air, me, hands, insignia; 'floor' not separately probed). The two new room Scenery entries answer (Deck Twelve 'corridor', Cargo Bay Entrance 'entrance'). The new spacetruck exterior object answers in the Cargo Bay: 'examine spacetruck' gives the authored description with the live hatch state, 'launch truck' -> 'You'\''re not even in it!', 'enter truck' with the hatch closed -> 'The spacetruck'\''s hatch is closed.' The SceneryItem namespace move (GameEngine.Location -> Model.Location) caused no regression: Zork's pre-existing North of House scenery still answers ('examine house', 'examine boards'). As PR #563 states, Zork and Planetfall gain the GlobalScenery capability without populating it, so 'examine wall' in Zork still falls through to the narrator -- expected, not a regression. Separately: 'take truck' in the Cargo Bay returns 'You can'\''t enter that.' (EnterSubLocationEngine.cs:49) rather than the item'\''s authored CannotBeTakenDescription 'It'\''s a twelve-meter spacecraft.' -- the prod parser classified a take as an enter intent; low, parser classification, not a #563 defect.

## AB-118 [INFO] 2.11.0 verified in prod: PR #572 Comm Room flask possession guard (issue #547)  · _open_

- game `planetfall` · area `Comm Room` · category `release-verification` · target_sha `3aa201c`
- command: `pour fluid into hole`

All five sub-cases of PR #572 confirmed live on prod 2.11.0. (1) Flask dropped in the Comm Room, pour -> 'You're not holding the flask.'; console stays black, flask stays full. (2) Flask left in Tower Core, pour -> engine falls through (quiet sentinel), remote flask NOT drained and enunciator NOT advanced - the puzzle-bypass/score-loss bug from #547 is gone. (3) Holding the flask still pours normally (gray light) - guard is a no-op on the happy path. (4) Flask stowed in the Patrol uniform pocket still pours - container-aware IsCarrying, not flat HasItem (#503 correction). (5) After a wrong-colour pour shuts the console down, the critical description again ends with the funnel-hole sentence.

## AB-120 [INFO] 2.12.0 verified in prod: PR #579 scenery reachable from the take path (issue #577)  · _open_

- game `zork` · area `North of House` · category `release-verification` · target_sha `0772234`
- command: `take house`

Zork I North of House. Before (per PR): 'get house' answered but 'take house' fell through to the narrator. On prod 2.12.0 both phrasings return the authored refusal, and the verb family agrees: take/get/grab/hold/acquire/snatch all give 'You can't take a house with you.' and take/get/pick up boards all give 'The boards won't budge.' Narrator-ON gives the same static string. Planetfall-side scenery was NOT independently exercised (covered only by the PR's own sweep test). Note: 'take house' at WEST of House still returns the sentinel, but that room declares no Scenery at all - a pre-existing content gap, not a #579 regression.
